...
const samplePrompts = [
  "Which categories drove margin growth?",
  "Why did the return rate change this period?",
  "What's driving the revenue trend this month?",
];
  ...
  return <AppShell active="loupe" brand="Loupe" brandIcon={ScanSearch} navigation={nav}>
    <div className="dashboard-surface">
      ...
      {error?<Unavailable message={error}/>:!data?<div className="card skeleton" aria-label="Loading live commerce intelligence"/>:<>
        {activeView==="overview"&&<>
          <section><div className="section-title">Key metrics</div><div className="metric-grid"><Stat label="Net revenue" value={money(data.revenue.value)} change={delta(data.revenue.change_pct)}/><Stat label="Gross margin" value={`${data.gross_margin_pct.value.toFixed(1)}%`} change={delta(data.gross_margin_pct.change_pct," pts")}/><Stat label="Order items" value={number(data.order_items.value)} change={delta(data.order_items.change_pct)}/><Stat label="Return rate" value={`${data.return_rate_pct.value.toFixed(1)}%`} change={delta(data.return_rate_pct.change_pct," pts")}/></div></section>
          <section><div className="section-title">Context</div><div className="insight-grid">
            <Card><div className="card-head"><div><h2>Executive summary</h2><div className="muted small">Grounded in current warehouse data</div></div><Sparkles size={18}/></div><p>{data.insight}</p></Card>
            <Card><div className="card-head"><div><h2>Source &amp; definition</h2><div className="muted small">Where these numbers come from</div></div><Badge tone={data.source_health.status==="healthy"?"accent":"warning"}>{data.source_health.status}</Badge></div>
        ...
        {activeView==="performance"&&<>
          <section><div className="section-title">Period change summary</div><Card><div className="card-head"><div><h2>vs. prior 30-day period</h2><div className="muted small">Change across the four governed metrics</div></div></div><div className="metric-grid">
